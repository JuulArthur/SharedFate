...
| Generator | 1,549 lines, about 190 of them blank. About 950 are reusable (atlas packer, mesh builder with UVs and weights, PNG writer, clip authoring, walk and stretch measurements, renders) and about 590 are mage-specific (painters 250, geometry and weights 150, poses 130, assembly). A full run takes 2.5-3.6 s including seven renders and the `.blend`. The atlas, glb and renders are byte-identical between runs. |
