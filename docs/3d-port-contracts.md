...
Actor bodies: `collision_layer` = 2, `collision_mask` = 1 + 8 = 9. Ground: layer 1, mask 0. Props: layer 8, mask 0. Pickup areas: layer 4, mask 0, `monitoring` off. Note (WP10): mask 9 does not include layer 2, so actors never collide with each other physically, as in 2D; spacing between actors is a navigation matter (avoidance and the approach rule below), not a physics one.
...
- Approach spacing (WP10): avoidance answers move a body only on frames it asked to move and never faster than `move_speed`; `hold_position()` parks the agent with a zero velocity; `get_attack_approach_distance(target)` is the one rule for where an attacker stops: `clamp(reach - margin, own radius + target radius + 0.1 m, reach - arrival slack)`. Enemy chase, player pursuit and the coordinator's engage and enemy-turn moves all use it, and actors hold position once within reach instead of re-aiming every refresh. Agents use `target_desired_distance` 0.1, `max_speed` = walk speed, `time_horizon_agents` 0.25. Diagnosis scene: `scenes/3d/tests/approach_test.tscn`.
...
| 2026-09-25 | WP12 | Wolf rigged (`Hips`, `Spine`, `Neck`, `Head`, `Tail`, four legs from `Hips`), `idle` 2.0 s and `trot` 0.4 s; measured stride 0.751 m per cycle gives `TROT_REFERENCE_SPEED` 1.878 m/s (foot slide 0.6 % in `enemy_test`); the wolf's `AnimationPlayer` runs in physics mode and pauses on death; wolf and prop glbs re-exported with one scene each (stray default `Cube` gone), navmesh unchanged; the `-colonly` and `<Name>_Body` conventions in section 10 still hold (full text in `docs/deviations/wp12.md`) | yes (10 note) |
| 2026-09-25 | WP11 | Souls rigged (knight 12 bones incl. `Cape`, rogue 11, mage 8 incl. `Robe`), rigid one-bone skinning, `idle` 2.0 s and `walk` 1.0 s clips in the glb; imported tree `<Name>/<Name>_Armature/Skeleton3D/<Name>_Body`, `HandPoint` and right weapon under a `BoneAttachment3D` `LowerArm_R`, `AnimationPlayer` at the root; the player drives the clips by ground speed with a 0.15 s cross-fade; the lead set the stride reference to the clip's real 1.92 m per cycle to stop foot slide; model `.import` files grow to about 4,000 lines from the loop setting; wolf and prop glbs still carry a stray default `Cube` scene until regenerated (full text in `docs/deviations/wp11.md`) | yes (10 note) |
| 2026-09-25 | WP10 | Actors never collided (mask 9 excludes layer 2; section 3 and the WP3c row corrected); the shoving was RVO avoidance splitting dodges plus stale server velocities; one approach rule on the base, hold-position within reach, agent tuning, enemy reach 1.3 m, player reach unchanged (full text in `docs/deviations/wp10.md`) | yes (3, 5.1, 5.3) |
| 2026-09-25 | WP9 | `scripts/3d/soul_bodies_3d.gd` on the player's `Model` holds all three bodies and shows the active soul's, moving `HandPoint` and `OverheadAnchor` to that body and copying the soul's weapon onto `EquippedWeaponMesh`; the soul decides the visible weapon, the item only damage, range and whether one shows; item grip values are replaced by fixed per-soul tilts; `knight_visual_3d.gd` deleted (full text in `docs/deviations/wp9.md`) | yes (5.2 note) |
