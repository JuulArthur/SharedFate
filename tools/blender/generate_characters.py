# Shared Fate 3D port, WP1: knight, rogue and mage as low-poly characters.
#
# Command line:
#     blender --background --python tools/blender/generate_characters.py -- --root <repo>
...
def build_mage_body():
    ...
    # robe: one tapered body from the hem to the shoulders
    cylinder(bm, (0, 0, 0.67), 0.40, 1.34, M_ROBE, segments=10, r_top=0.17)
...
def assemble(label, body, cape, weapons, hand_point):
    """Link one character, add its empties and export it. `weapons` is a list of
    (object, location, rotation_euler) triples."""
    coll = model_collection("SF_" + label.capitalize())
    link(coll, body)
    objects = [body]
    if cape is not None:
        link(coll, cape)
    ...
    hand = add_empty("HandPoint", hand_point, parent=body, collection=coll)
    ...
    export_glb(objects, os.path.join(MODEL_DIR, label + ".glb"),
               root_name=label.capitalize())
    render_preview(objects, os.path.join(PREVIEW_DIR, label + ".png"))

    # Blender object names are unique per file, so free "HandPoint" and
    # "OverheadAnchor" for the next character now that this one is exported.
    hand.name = label.capitalize() + "_HandPoint"
    overhead.name = label.capitalize() + "_OverheadAnchor"
    return objects
...
def main():
    ...
    try:
        assemble("knight", build_knight_body(), build_knight_cape(),
                 [(build_sword(), (0.335, 0.08, 0.86), (math.pi, 0.0, 0.0))],
                 (0.335, 0.08, 0.86))
        assemble("rogue", build_rogue_body(), None,
                 [(build_dagger("Dagger_R"), (0.265, 0.06, 0.83), (math.pi, 0.0, 0.0)),
                  (build_dagger("Dagger_L"), (-0.265, 0.06, 0.83), (math.pi, 0.0, 0.0))],
                 (0.265, 0.06, 0.83))
        assemble("mage", build_mage_body(), None,
                 [(build_staff(), (0.245, 0.06, 0.92), (0.0, 0.0, 0.0))],
                 (0.245, 0.06, 0.92))
    finally:
        restore_scene(prev)
